...
<html lang="en">
    <head>
        ...
        <style>
            ...
            .graph-legend{position:absolute;bottom:0;left:0;right:0;z-index:1000;display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap;background:rgba(255,255,255,.85);border-top:1px solid #e0e0e0;padding:3px 10px;font-size:12px;color:#333}
            .legend-item{display:flex;align-items:center;gap:5px;line-height:18px}
    ...
    <div class="main-content" id="graph-section">
        ...
        <div id="graph-container">
            <div class="graph-legend" id="graph-legend" style="display:none;">
                <div class="legend-item"><span class="legend-swatch" id="legend-in"></span>In</div>
                <div class="legend-item"><span class="legend-swatch" id="legend-out"></span><span id="legend-out-label">Out</span></div>
                <div class="legend-item" id="legend-undec-item"><span class="legend-swatch" id="legend-undec"></span>Undec</div>
                <label class="legend-switch"><input type="checkbox" id="legend-distinguish-toggle">Distinguish Out/Undec</label>
            </div>
...
function getScriptFuncs(): string {
    ...
    function getThemeColors() {
        ...
        switch (theme) {
            ...
            default:
                return {
                    'default': 'fill:#D5E8D4,stroke:#000000',
                    add: 'fill:#D5E8D4,stroke:#000000,stroke-width:4px',
                    select: 'fill:#0F7C0F,stroke:#000000,color:#ffffff',
                    selectadd: 'fill:#0F7C0F,stroke:#000000,color:#ffffff,stroke-width:4px',
                    undec: 'fill:#FFFFFF,stroke:#000000',
                    undecadd: 'fill:#FFFFFF,stroke:#000000,stroke-width:4px',
                    out: 'fill:#D5E8D4,stroke:#000000',
                    outadd: 'fill:#D5E8D4,stroke:#000000,stroke-width:4px',
    ...
    function distinguishOutUndec() {
        return sessionStorage.getItem('argblazer_global_sessionDistinguishOutUndec') === 'true';
    }
    ...
    function updateLegend() {
        ...
        var hasLabelling = colors && nodeData && nodeData.some(function(node) { return colors[node.id]; });
        legend.style.display = hasLabelling ? '' : 'none';
        if (!hasLabelling) return;

        var split = distinguishOutUndec();
        document.getElementById('legend-out-label').textContent = split ? 'Out' : 'Not in';
        document.getElementById('legend-undec-item').style.display = split ? '' : 'none';
        var distinguishToggle = document.getElementById('legend-distinguish-toggle');
        if (distinguishToggle) distinguishToggle.checked = split;

        var themeColors = getThemeColors();
        [['legend-in', themeColors.select], ['legend-out', themeColors.out], ['legend-undec', themeColors.undec]].forEach(function(entry) {
            var swatch = document.getElementById(entry[0]);
            var fillMatch = entry[1].match(/fill:([^,]+)/);
            var strokeMatch = entry[1].match(/stroke:([^,]+)/);
            if (swatch && fillMatch && strokeMatch) {
                swatch.style.background = fillMatch[1];
                swatch.style.borderColor = strokeMatch[1];
            }
        });
    }
    ...
    async function render() {
        ...
        nodeData.forEach(function(node) {
            ...
            } else if (colors[node.id] === 'undec' && distinguishOutUndec()) {
                if (newNodeIds.has(node.id)) {
                    undecAddNodes += undecAddNodes ? ',' + node.id : node.id;
                } else {
                    undecNodes += undecNodes ? ',' + node.id : node.id;
                }
            } else if (colors[node.id] === 'out' || colors[node.id] === 'undec') {
                if (newNodeIds.has(node.id)) {
    ...
    // Labels every argument as "in" (the extension itself), "out" (attacked by any argument in the extension) or "undec" (neither), following the standard labeling semantics.
    function computeLabelling(nodeIds) {
        ...
        nodeData.forEach(function(node) {
            if (inSet.has(node.id)) {
                labeling[node.id] = 'selected';
            } else if (outSet.has(node.id)) {
                labeling[node.id] = 'out';
            } else {
                labeling[node.id] = 'undec';
            }
        });
        return labeling;
    }
    ...
    function setupControls() {
        ...
        var distinguishToggle = document.getElementById('legend-distinguish-toggle');
        if (distinguishToggle) {
            distinguishToggle.addEventListener('change', function() {
                sessionStorage.setItem('argblazer_global_sessionDistinguishOutUndec', distinguishToggle.checked ? 'true' : 'false');
                render();
            });
        }
